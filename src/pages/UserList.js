...
import { AllUsers, blockUnblock, deleteAccount, getAllUsers } from '../store/slices/userSlice';
...
const UserList = () => {
    ...
    var csvData = [
        ["Name", "Email", "Verified", "Block", "Email Notification", "In App Notification", "Text Notification", "Role"],
    ]
    users?.map((item) => 
        csvData.push([`${item?.name}`, `${item?.email}`, `${item?.isActive}`, `${item?.block}`, `${item?.emailNotification}`, `${item?.inAppNotification}`, `${item?.textNotification}`, `${item?.role}`])
    )
    const openModal = (id) => {
        setSwitchBtn2(true)
        setId(id)
